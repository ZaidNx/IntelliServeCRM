...
export interface IStorage {
  ...
  createPublicBooking(slug: string, booking: BookingRequest): Promise<Appointment>;
}
...
export class MongoStorage implements IStorage {
  ...
  async createUser(userData: InsertUser): Promise<User> {
    ...
    const baseSlug = userData.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  ...
  async getServicesByUserId(userId: string): Promise<Service[]> {
    const db = getDatabase();
    return await db.collection('services').find({ userId: new ObjectId(userId) }).toArray();
  }
  ...
  async getCustomersByUserId(userId: string): Promise<Customer[]> {
    ...
    const appointments = await db.collection('appointments').find({ userId: new ObjectId(userId) }).toArray();
  ...
  async getAppointmentsByUserId(userId: string): Promise<Appointment[]> {
    const db = getDatabase();
    return await db.collection('appointments').find({ userId: new ObjectId(userId) }).toArray();
  }
  ...
  // Public booking
  async createPublicBooking(slug: string, booking: BookingRequest): Promise<Appointment> {
