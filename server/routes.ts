...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  if (professions.length === 0) {
    await storage.createProfession({ name: 'Barber', icon: 'fas fa-cut', description: 'Hair cutting & styling' });
    await storage.createProfession({ name: 'Dentist', icon: 'fas fa-tooth', description: 'Dental care & treatment' });
    await storage.createProfession({ name: 'Beautician', icon: 'fas fa-spa', description: 'Beauty & wellness' });
    await storage.createProfession({ name: 'Mechanic', icon: 'fas fa-wrench', description: 'Auto repair services' });
  }
  ...
  app.put('/api/services/:id', authenticateToken, async (req: AuthRequest, res) => {
    try {
  ...
  app.put('/api/appointments/:id', authenticateToken, async (req: AuthRequest, res) => {
    try {
  ...
  app.post('/api/public/:slug/book', async (req, res) => {
    try {
      ...
      const appointment = await storage.createPublicBooking(req.params.slug, bookingData);
      res.status(201).json(appointment);
    } catch (error: any) {
      res.status(400).json({ message: error.message });
    }
    ...
    try {
      ...
      if (response.action === 'book_appointment' && response.actionData) {
        try {
          ...
          if (availability.available) {
            const appointment = await storage.createPublicBooking(req.params.slug, response.actionData);
          ...
          response.response += " However, there was an error processing your booking. Please try again.";
        }
      }
  ...
  app.post('/api/contact', async (req, res) => {
    try {
      ...
      if (emailSent) {
        res.json({ message: 'Message sent successfully! You will receive a copy in your email.' });
      } else {
        res.json({ message: 'Message received successfully. Email notification failed but your message is saved.' });
      }
    } catch (error: any) {
  ...
  app.get('/api/dashboard/stats', authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const newCustomers = customers.filter(c => c.createdAt && c.createdAt.toISOString().split('T')[0] >= startOfMonth);
      
      const completedAppointments = appointments.filter(a => a.status === 'Completed');
      const revenue = completedAppointments.reduce((sum, appointment) => {
        const service = services.find(s => s._id?.toString() === appointment.serviceId.toString());
